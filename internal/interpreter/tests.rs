...
fn interpreter_smoke() {
    ...
    let def = result.components.get("Greeter").expect("Greeter should be compiled");
...
fn interpreter_callbacks_and_math() {
    ...
    let def = result.components.get("Calc").expect("Calc should compile");
    let instance = def.create();
...
/// A function on a sub-component returns an enum value by falling
/// through empty `if` branches (mirrors the Issue4070 pattern
...
/// A binding annotated with `animate` must not crash creation.
#[test]
...
fn interpreter_repeater() {
    ...
    let def = result.components.get("List").expect("List should compile");
...
fn interpreter_two_way_binding() {
    ...
    let def = result.components.get("Form").expect("Form should compile");
    let instance = def.create();
...
fn interpreter_conditional() {
    ...
    let def = result.components.get("Gate").expect("Gate should compile");
    let _instance = def.create();
}
...
fn interpreter_globals() {
    ...
    let def = result.components.get("App").expect("App should compile");
    let instance = def.create();
...
/// Smoke-test a TouchArea click via the testing backend: the test-only
/// entry points must attach the window and dispatch into the item tree.
/// Equivalent to `test_nodejs_elements_toucharea`, but runnable without
/// the node harness.
#[test]
fn interpreter_toucharea_click() {
    ...
    let def = result.components.get("TestCase").expect("TestCase should compile");
    let instance = def.create();
    let public = crate::api::ComponentInstance { inner: instance.clone() };
    let adapter = public.inner.window_adapter_ref().unwrap().clone();

    // Click outside the TouchArea first — should not fire.
    i_slint_backend_testing::testing_backend::send_mouse_click(5.0, 5.0, &adapter);
    assert_eq!(instance.get_property("hits"), Some(Value::Number(0.0)));

    // Click inside — should fire once.
    i_slint_backend_testing::testing_backend::send_mouse_click(105.0, 105.0, &adapter);
    assert_eq!(instance.get_property("hits"), Some(Value::Number(1.0)));
}

/// Regression test mirroring the gallery: a shared `Page` base with a
/// conditional header widget plus an always-present one, embedded in outer
/// `if` conditionals switched by an integer property. The always-present
/// widget on each page must read its own literal string, not a stale one
/// leaked from a previously visited page.
#[test]
...
/// `find_by_element_type_name` (and anything else using the vtable's
/// `item_element_infos`) must find widgets whose element-info entry lives
/// deep in a sub-component path: an always-visible `Switch` declared after
/// a conditional `Switch` in the same parent layout must still be
...
fn interpreter_switch_conditional_pages_labels() {
    ...
    // Look up a page's Text by its accessible-label; whether it is found
    // tells which page is materialized.
    let find_text_by_label = |label: &str| -> Option<String> {
        ...
        // Text is a native item, so its `text` property isn't reachable
        // through the public API — return the accessible-label instead.
...
fn two_way_to_model_skips_identical_writes() {
    ...
    let row: crate::api::Struct = [("v".to_string(), Value::Number(42.))].into_iter().collect();
    let model = std::rc::Rc::new(CountingModel {
    ...
    i_slint_backend_testing::testing_backend::send_mouse_click(50.0, 50.0, &adapter);
    assert_eq!(model.sets.get(), 0);

    // A different value must.
    i_slint_backend_testing::testing_backend::send_mouse_click(150.0, 50.0, &adapter);
