...
pub(crate) fn solve_flexbox_layout_with_measure(ctx: &mut EvalContext, expr: &Expression) -> Value {
    ...
    // Preferred (default-constraint) size per cell, used when taffy asks for
    // a dimension without a known cross-axis size.
    ...
    for item in default_cells {
        ...
        // Repeater cells (the `Right` case) are not emitted; they fall
        // through to 0 below.
    }
    ...
    let mut measure = |index: usize, known_w: Option<f32>, known_h: Option<f32>| -> (f32, f32) {
        ...
        if known_w.is_some()
            && known_h.is_none()
            && let Some(itertools::Either::Left((_, v_info))) = measure_cells.get(index)
        {
            ctx.locals.insert("measure_known_w".into(), Value::Number(w as f64));
            let nh = eval_info(ctx, v_info).preferred_bounded();
            return (w, nh);
        }
        if known_h.is_some()
            && known_w.is_none()
            && let Some(itertools::Either::Left((h_info, _))) = measure_cells.get(index)
        {
