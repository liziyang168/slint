...
fn field_default(
    ...
    if let Some(declared) = declared {
        ...
        return match i_slint_common::builtin_structs::parse_builtin_struct_field_default(declared) {
            // Enum values are kebab-case strings in the JS API
            BuiltinStructFieldDefault::EnumValue { variant, .. } => {
                format!("\"{}\"", to_kebab_case(&variant))
            }
            BuiltinStructFieldDefault::Bool(b) => b.to_string(),
            // Number literals are the same in JS
